...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, FASFFHead, AFPNHead, DynamicHead, DynamicDCNv3Head)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
...
#-----------------------------+repvit------------------------------------
        ...
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, FASFFHead, AFPNHead, DynamicHead, DynamicDCNv3Head}:
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if m == "obb":
            return "obb"
        else:                      # +    原因：这里的m在代码执行过程中会将代码自动转换为小写，所以直接else更为方便
            return 'detect'
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, v10Detect, FASFFHead, AFPNHead, DynamicHead, DynamicDCNv3Head)):
                return "detect"
